...
public class AudioClip {
				...
				do {
					...
					try {
						...
						if (isClipInMemory) {
							...
							int nBytesRead = 0;
							while ((nBytesRead = audioStream.read(sampledDataBuffer, 0,
									sampledDataBuffer.length)) != -1) {
	...
	private static class MidiAudioClip implements InternalAudioClip, MetaEventListener {
		...
		public void playAsActionSound() {
			throw new RuntimeException(
					"Operation is not supported. MIDI sequence cannot be played as an action sound.");
		}
	...
	public AudioClip(String audioLocation, boolean storeInMemory) {
		...
		if (audioResourceUrl == null)
			throw new RuntimeException(
					"Parameter audioLocation (" + audioLocation + ") does not refer to an audio file.");
